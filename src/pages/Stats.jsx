...
import { Card, CardTitle, Grid, MetricCard, Pill } from '../components/UI'

function CustomTooltip({ active, payload, label, p }) {
  ...
  return (
    <div style={{ background: p.bg2, border: `1px solid ${p.br}`, borderRadius: 8, padding: '8px 12px', fontSize: 11 }}>
      ...
      {payload.map((entry, i) => (
        <div key={i} style={{ color: entry.color || p.ac, marginBottom: 2 }}>
...
export default function Stats() {
  ...
  const [activeMetric, setActiveMetric] = React.useState('reach')

  const totals = {
    reach: data.reduce((a, x) => a + x.reach, 0),
    eng: data.reduce((a, x) => a + x.eng, 0),
    follows: data.reduce((a, x) => a + x.follows, 0),
  }
  ...
  return (
    <div className="animate-up">
      <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
        {['weekly', 'monthly'].map(r => (
          <button key={r} onClick={() => setAnalyticsRange(r)}
            style={{ padding: '5px 14px', borderRadius: 6, border: `1px solid ${analyticsRange === r ? p.ac : p.br}`, background: analyticsRange === r ? p.ac + '22' : 'transparent', color: analyticsRange === r ? p.ac : p.tx, cursor: 'pointer', fontSize: 11, fontWeight: 600, transition: 'all .2s', textTransform: 'capitalize' }}>
            {r}
          </button>
        ))}
      </div>
      ...
      <Card style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {['reach', 'eng', 'follows'].map(m => (
              <button key={m} onClick={() => setActiveMetric(m)}
                style={{ padding: '3px 10px', borderRadius: 6, border: `1px solid ${activeMetric === m ? p.ac : p.br}`, background: activeMetric === m ? p.ac + '22' : 'transparent', color: activeMetric === m ? p.ac : p.tx, cursor: 'pointer', fontSize: 10, fontWeight: 600, transition: 'all .2s', textTransform: 'capitalize' }}>
                {m}
              </button>
            ))}
          </div>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
            ...
            <Area type="monotone" dataKey={activeMetric} stroke={p.ac} strokeWidth={2} fill="url(#areaGrad)" dot={{ fill: p.ac, strokeWidth: 0, r: 3 }} activeDot={{ r: 5, fill: p.ac }} />
          </AreaChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Grid cols={2} gap={10} style={{ marginBottom: 14 }}>
        <Card>
          <CardTitle>Platform Reach Comparison</CardTitle>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={analytics.platforms} layout="vertical" margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
              ...
              <Tooltip content={<CustomTooltip p={p} />} formatter={v => v.toLocaleString()} />
              <Bar dataKey="reach" radius={[0, 4, 4, 0]} fill={p.ac} opacity={0.8} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        <Card>
          <CardTitle>6-Month Follower Growth</CardTitle>
          <ResponsiveContainer width="100%" height={160}>
