...
const STEPS = [
  { title: 'Color Grade', desc: 'Boost shadows +15, saturation +10. TikTok: high contrast. IG: muted tones. YouTube: punchy but natural. Match your brand palette consistently.' },
  { title: 'Crop & Frame', desc: 'Rule of thirds. TikTok 9:16 — avoid top/bottom 15%. YouTube thumbnail 16:9 — faces left 40%. Instagram 4:5 for max feed real estate.' },
  { title: 'Text Overlay', desc: 'Max 5 words. Bold weight. White + 2px black stroke. Center or bottom third. Min 48pt export size. Squint test: readable blurred? If not, resize.' },
  { title: 'Caption Formula', desc: 'Hook → Value → CTA. 150 chars max on X. Pinned comment for TikTok long copy. 3-5 niche hashtags IG. First line decides the click.' },
  { title: 'Audio Layer', desc: 'Music 15-20% volume. Voice 80-85%. Trending audio +40% reach. Source: Epidemic Sound or Artlist. No dead air — ambient sound fills gaps.' },
  { title: 'Export & Post', desc: 'Verify optimal window before publishing. TikTok: 7-9am, 12-3pm, 7-11pm. IG: Tue-Thu 9am-12pm. YT: 2-4pm weekdays. Schedule 3-5 min before peak.' },
]
      ...
      <Grid cols={2} gap={10}>
        ...
        <div>
          <Card style={{ marginBottom: 10 }}>
            ...
            <Grid cols={2} gap={6}>
              {[['TikTok','9:16 · 1080×1920 · 60s max · MP4'],['Instagram','4:5 · 1080×1350 · 2200 chars · JPG/MP4'],['YouTube','16:9 · 1920×1080 · 15min+ · MP4'],['X / Twitter','16:9 · 1280×720 · 140 chars · MP4']].map(([pl, spec]) => (
          ...
          <Card>
            ...
            {['"Nobody talks about this trick..."','"I tried this for 30 days. Here\'s what happened."','"Stop doing this. Do this instead."','"The algorithm is pushing this right now."','"POV: You finally understand this concept"'].map((hook, i) => (
              <div key={i} style={{ padding: '8px 10px', borderRadius: 8, border: `1px solid ${p.br}`, marginBottom: 6, fontSize: 11, opacity: .8, cursor: 'pointer', transition: 'all .2s', background: p.bg }}
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor = p.br; e.currentTarget.style.opacity = '.8' }}>
                {hook}
              </div>
            ))}
          </Card>
...
export function Settings() {
  const { palette: p } = useStore()
  const [toggles, setToggles] = React.useState({ trends: true, reminders: true, insights: false, warnings: true, autoSchedule: false, optimalTime: true })
  const toggle = key => setToggles(t => ({ ...t, [key]: !t[key] }))

  function Switch({ k }) {
    const on = toggles[k]
    return (
      <div onClick={() => toggle(k)} style={{ width: 32, height: 18, borderRadius: 9, background: on ? p.ac : p.br, position: 'relative', cursor: 'pointer', transition: 'background .2s', flexShrink: 0 }}>
        <div style={{ width: 14, height: 14, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, transition: 'all .2s', left: on ? 'auto' : 2, right: on ? 2 : 'auto' }} />
      </div>
    )
  }
      ...
      <Grid cols={2} gap={10}>
        <div>
            ...
            {[['YT','YouTube','@creator_channel','#3b82f6',true],['TT','TikTok','@fossarythm8','#00b894',true],['IG','Instagram','@fossarythm8','#d946ef',true],['X','X / Twitter','@fossarythm8_','#6366f1',false]].map(([ic,n,h,c,conn]) => (
              <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${p.br}` }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 10, opacity: .4 }}>{h}</div>
                </div>
          ...
          <Card>
            ...
            {[['Fast','claude-haiku-4','Fastest, lower cost'],['Balanced','claude-sonnet-4','Best quality/speed'],['Max','claude-opus-4','Highest quality']].map(([tier,model,desc],i) => (
              <div key={tier} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < 2 ? `1px solid ${p.br}` : 'none' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 600 }}>{tier} <span style={{ fontSize: 10, opacity: .4, fontFamily: "'Space Mono',monospace" }}>· {model}</span></div>
                  <div style={{ fontSize: 10, opacity: .4, marginTop: 2 }}>{desc}</div>
                </div>
        ...
        <div>
            ...
            {[['trends','Trend alerts','Notify when a hashtag spikes'],['reminders','Post reminders','Alert before optimal windows'],['insights','AI insights','Daily performance digest'],['warnings','Auto-post warnings','Alert before posts fire']].map(([k,title,desc]) => (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${p.br}` }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 10, opacity: .4 }}>{desc}</div>
                </div>
                <Switch k={k} />
              </div>
            ))}
          ...
          <Card>
            ...
            {[['autoSchedule','Auto-schedule mode','Automatically queue at optimal times (BETA)'],['optimalTime','Optimal time selection','Override manual times with AI-recommended windows']].map(([k,title,desc]) => (
            ...
            <div style={{ marginTop: 14, padding: 10, borderRadius: 8, background: p.ac + '0d', border: `1px solid ${p.ac}22`, fontSize: 11, opacity: .7 }}>
              FOSSARYTHM8 v1.0.0 · Open source · MIT License
            </div>
          </Card>
        </div>
      </Grid>
