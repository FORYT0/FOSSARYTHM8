...
const PLT_LABELS = { tiktok:'TikTok', youtube:'YouTube', instagram:'Instagram', x:'X / Twitter' }
const PLT_COLORS = { tiktok:'#00b894', youtube:'#3b82f6', instagram:'#d946ef', x:'#6366f1' }
const ALG_TIPS = {
  youtube:   ['Post 2-3x/week — consistency is the #1 signal','Thumbnail CTR target: 10%+, test faces vs text','Hook must deliver on title promise in first 30s','Chapters boost watch time and search indexing','End screen + cards at 85% retention point drives subs'],
  tiktok:    ['Post 3-4x/day across different time zones','Trending audio boosts reach by up to 40%','Hook must land in first 3 seconds — zero intros','Reply to every comment within 1hr to boost velocity','Duet or stitch trending content to borrow audience'],
  instagram: ['Carousels get 3x more saves than single images','Reels under 60s hit optimal distribution','Mix 3 niche + 2 trending hashtags per post','Stories keep you in followers daily feed — post daily','Alt text and caption keywords feed Instagram search'],
  x:         ['Text-only threads outperform link posts','Quote-tweet established accounts for instant visibility','B2B peaks 8-10am, consumer content peaks 6-8pm','Bookmarks now signal quality to the algorithm','Jump on trending threads within the first 2 hours'],
}
...
export default function Trends() {
  ...
  React.useEffect(() => {
    if (selectedTrend) {
      ...
      setDetail(selectedTrend)
      setSelectedTrend(null)
    }
  }, [])

  const data = trends[platform] || []
  const color = PLT_COLORS[platform]

  return (
    <div className="animate-up">
      <div style={{ display:'flex', gap:6, marginBottom:16, flexWrap:'wrap' }}>
        {Object.keys(PLT_LABELS).map(pl => (
          <button key={pl} onClick={()=>setPlatform(pl)}
            style={{ padding:'6px 14px', borderRadius:6, border:`1px solid ${platform===pl?PLT_COLORS[pl]:p.br}`, background:platform===pl?PLT_COLORS[pl]+'22':'transparent', color:platform===pl?PLT_COLORS[pl]:p.tx, cursor:'pointer', fontSize:11, fontWeight:600, transition:'all .2s' }}>
            {PLT_LABELS[pl]}
          </button>
        ))}
      </div>
      <Grid cols={2} gap={10}>
        <Card>
          ...
          {data.map((t,i) => (
            <div key={t.tag} onClick={()=>setDetail({...t,platform})}
        ...
        <Card>
          <CardTitle>Algorithm Intelligence · {PLT_LABELS[platform]}</CardTitle>
          {(ALG_TIPS[platform]||[]).map((tip,i) => (
      ...
      <Modal open={!!detail} onClose={()=>setDetail(null)} title={`TREND ANALYSIS — ${detail?.tag}`} width={680}>
        {detail && (
          <div>
            <Grid cols={3} gap={8} style={{ marginBottom:16 }}>
              {[['Trend Score',detail.score,p.ac],['Volume',detail.vol,'#22c55e'],['Growth',detail.delta,'#f59e0b']].map(([l,v,c])=>(
                <div key={l} style={{ padding:12, borderRadius:8, background:p.bg, border:`1px solid ${p.br}`, textAlign:'center' }}>
                  <div style={{ fontFamily:"'Space Mono',monospace", fontSize:20, fontWeight:700, color:c }}>{v}</div>
                  <div style={{ fontSize:10, opacity:.4, letterSpacing:1, textTransform:'uppercase', marginTop:3 }}>{l}</div>
                </div>
              ))}
            </Grid>
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:'flex', alignItems:'flex-end', gap:6, height:80 }}>
                {(detail.growth||[]).map((v,i)=>{
                  const max=Math.max(...detail.growth), h=Math.round((v/max)*70)
                  return (
                    <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                      <div style={{ width:'100%', height:h, borderRadius:'3px 3px 0 0', background:PLT_COLORS[detail.platform]||p.ac, opacity:i===6?1:.45 }} />
            ...
            <div style={{ padding:14, borderRadius:10, background:p.bg, border:`1px solid ${p.br}`, marginBottom:16 }}>
              <div style={{ fontSize:11, lineHeight:1.7, opacity:.85 }}>{detail.desc}</div>
            </div>
            <Grid cols={2} gap={10} style={{ marginBottom:16 }}>
              <div style={{ padding:12, borderRadius:8, background:p.bg, border:`1px solid ${p.br}` }}>
                ...
                <div style={{ fontSize:16, fontWeight:700, color:p.ac, fontFamily:"'Space Mono',monospace" }}>{detail.engagement}</div>
              </div>
              <div style={{ padding:12, borderRadius:8, background:p.bg, border:`1px solid ${p.br}` }}>
                ...
                <div style={{ fontSize:13, fontWeight:700, color:p.ac }}>{detail.peakTime}</div>
              </div>
            </Grid>
            <Button onClick={()=>setDetail(null)} style={{ width:'100%' }}>Close</Button>
          </div>
        )}
      </Modal>
    </div>
